...
function pathToUri(relativePath) {
    const parts = relativePath.split('/');
    const encoded = parts.map(p => encodeURIComponent(p)).join('/');
    return `switch-docs://${encoded}`;
}
function uriToPath(uri) {
    if (!uri.startsWith('switch-docs://'))
        return null;
...
function generateResourceName(relativePath) {
    const filename = path_1.default.basename(relativePath, '.md');
...
async function generateResourceDescription(absPath) {
    try {
