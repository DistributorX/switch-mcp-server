...
export interface DocChunk {
  ...
  source: string;
  /** Section heading (or file title for intro chunks) */
  heading: string;
  /** The text content of this chunk */
  text: string;
  /** SHA-256 hash of the text (for cache invalidation) */
  hash: string;
...
export function pathToUri(relativePath: string): string {
  const parts = relativePath.split(path.sep);
  return "switch-docs://" + parts.map(p => encodeURIComponent(p)).join("/");
}

export function generateName(relativePath: string): string {
  const filename = path.basename(relativePath, ".md");
  const chapterMatch = filename.match(/Chapter\s+(\d+)\s*-\s*(.+)/i);
  if (chapterMatch) return `Chapter ${chapterMatch[1]}: ${chapterMatch[2].trim()}`;
  return filename
    .replace(/[-_]/g, " ")
    .split(" ")
...
export function discoverDocs(docRoot: string): DocFile[] {
...
export function chunkMarkdown(content: string, relativePath: string, uri: string): DocChunk[] {
  ...
  const rawChunks: Array<{ heading: string; text: string }> = [];
 ...
 */
function splitByParagraphs(text: string, heading: string): Array<{ heading: string; text: string }> {
  const subChunks: Array<{ heading: string; text: string }> = [];
  let partNum = 1;

  function flush(t: string) {
...
function splitByLines(text: string, flush: (t: string) => void) {
...
export function chunkAllDocs(docRoot: string, docFiles: DocFile[]): DocChunk[] {
