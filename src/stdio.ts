...
import { discoverDocs, chunkAllDocs, pathToUri } from "./docs.js";
...
for (const doc of docFiles) {
  server.resource(
    doc.relativePath,
    doc.uri,
    { title: doc.displayName, description: doc.description, mimeType: "text/markdown" },
    async (uri) => {
...
server.tool(
  "search_docs",
  "Search Switch scripting documentation using semantic search. Understands natural language queries like 'how do I read XML data from a job' as well as specific terms like 'XmlDocument'.",
  {
  ...
  async ({ query, limit }) => {
    ...
    if (results.length === 0) {
      return {
        content: [{ type: "text" as const, text: `No results found for "${query}". Try rephrasing your question.` }],
      };
    }
...
function parseArgs(argv: string[]) {
  ...
  for (let i = 0; i < argv.length; i++) {
    const a = argv[i];
    if (a === "--doc-root") out.docRoot = argv[++i];
